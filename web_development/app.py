...
def home():
    if request.accept_mimetypes.best == "application/json":
        return (
            jsonify({"message": "Go to /osas/api/admin/login for authentication"}),
            200,
        )
...
def osas_root():
    if request.accept_mimetypes.best == "application/json":
        return (
            jsonify(
                {"message": "Go to /osas/api/admin/login or /osas/api/auth_status"}
            ),
            200,
        )
    return redirect(url_for("osas.osas_login"))
...
def pres_root():
    if request.accept_mimetypes.best == "application/json":
        return (
            jsonify(
                {"message": "Go to /pres/api/admin/login or /pres/api/auth_status"}
            ),
            200,
        )
